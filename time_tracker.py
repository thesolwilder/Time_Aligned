...
class TimeTracker:
    ...
    def create_widgets(self):
        ...
        # Sphere selection
        ttk.Label(main_frame, text="Sphere:").grid(row=0, column=0, sticky=tk.W, pady=5)
        self.sphere_var = tk.StringVar(value=self.settings["default_sphere"])
        self.sphere_entry = ttk.Entry(main_frame, textvariable=self.sphere_var, width=30)
        self.sphere_entry.grid(row=0, column=1, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        ...
        self.end_button = ttk.Button(
            button_frame, text="End Session", command=self.end_session, state=tk.DISABLED
        )
        self.end_button.grid(row=0, column=1, padx=5)
        
        self.break_button = ttk.Button(
            button_frame, text="Start Break", command=self.toggle_break, state=tk.DISABLED
        )
    ...
    def start_session(self):
        ...
        if not sphere:
            messagebox.showwarning("Warning", "Please enter a sphere name")
            return
        ...
        self.session_name = f"{sphere}_{int(self.session_start_time)}"
        ...
        self.sphere_entry.config(state=tk.DISABLED)
    ...
    def end_session(self):
        ...
        self.sphere_entry.config(state=tk.NORMAL)
        self.status_label.config(text="Session ended")
        
        messagebox.showinfo("Session Ended", f"Total time: {self.format_time(total_elapsed)}")
        ...
        if not self.break_active:
            ...
            if self.session_name in all_data:
                all_data[self.session_name]["breaks"].append({
                    "start": datetime.fromtimestamp(self.break_start_time).strftime("%H:%M:%S"),
        ...
        if not self.session_idle and idle_time >= self.settings["idle_threshold"]:
            ...
            if self.session_name in all_data:
                all_data[self.session_name]["idle_periods"].append({
                    "start": datetime.fromtimestamp(self.idle_start_time).strftime("%H:%M:%S"),
    ...
    def on_closing(self):
        ...
        if self.session_active:
            if messagebox.askokcancel("Quit", "A session is active. Do you want to end it and quit?"):
                self.end_session()
                self.stop_input_monitoring()
                self.root.destroy()
        else:
