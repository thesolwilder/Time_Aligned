...
from test_helpers import (
    TestDataGenerator, TestFileManager,
    assert_duration_accurate, assert_session_structure_valid
)
...
class TestTimeTrackingAccuracy(unittest.TestCase):
    ...
    def test_session_start_creates_timestamp(self):
        ...
        self.assertIsNotNone(tracker.start_timestamp)
        
        # Timestamp should be close to now
        self.assertLess(abs(tracker.start_timestamp - start_time), 1.0)
        
        tracker.stop_session()
        root.destroy()
    
    def test_session_saves_data_on_stop(self):
        """Test that stopping session saves data to file"""
        from time_tracker import TimeTracker
        import tkinter as tk
    ...
    def test_active_duration_recorded(self):
        ...
        assert_duration_accurate(self, expected_duration, session["active_duration"], tolerance=0.5)
